	...
	jQuery(document).ready(function($) {
		...
		$('#post-types a').click(function(e) {
			...
			if ($(this).attr('id') == 'post') {
				$('#posttitle').val("<?php echo esc_js( __('Discussion Topic', 'p2') ); ?>");
			} else {
				$('#posttitle').val('');
			}
...
<div id="postbox">
		<ul id="post-types">
			<li><a id="post" class="post-format-button<?php if ( 'post' == $post_format || 'standard' == $post_format ) : ?> selected<?php endif; ?>" href="<?php echo site_url( '?p=post' ); ?>" title="<?php esc_attr_e( 'New Discussion', 'p2' ); ?>"><?php _e( 'New Discussion', 'p2' ); ?></a></li>
			</ul>
			...
			<form id="new_post" name="new_post" method="post" action="<?php echo site_url(); ?>/">
				...
				<div id="postbox-type-post" class="post-input <?php if ( 'post' == $post_format || 'standard' == $post_format ) echo ' selected'; ?>">
					<input type="text" name="posttitle" id="posttitle" value=""
						onfocus="this.value=(this.value=='<?php echo esc_js( __( 'Discussion Topic', 'p2' ) ); ?>') ? '' : this.value;"
						onblur="this.value=(this.value=='') ? '<?php echo esc_js( __( 'Discussion Topic', 'p2' ) ); ?>' : this.value;" />
				</div>
